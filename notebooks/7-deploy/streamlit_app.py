...
def render_customer_lookup(customer_table: pd.DataFrame) -> None:
    ...
    st.dataframe(row[available_cols].to_frame("valor"), use_container_width=True)
    st.success(segment_recommendation(segment, probability))


def default_value(churn_dataset: pd.DataFrame, column: str, fallback: float) -> float:
    if column in churn_dataset.columns:
        return float(churn_dataset[column].median())
    return fallback


def collect_simulation_inputs(churn_dataset: pd.DataFrame) -> dict[str, float]:
    st.write("Cargar valores del cliente a evaluar.")
    col1, col2, col3 = st.columns(3)
    values: dict[str, float] = {}
    with col1:
        values["Recency"] = st.number_input(
            "Recency", 0.0, 500.0, default_value(churn_dataset, "Recency", 90.0)
        )
        values["Frequency"] = st.number_input(
            "Frequency", 0.0, 500.0, default_value(churn_dataset, "Frequency", 3.0)
        )
        values["Monetary"] = st.number_input(
            "Monetary", 0.0, 100000.0, default_value(churn_dataset, "Monetary", 500.0)
        )
        values["Cancel_rate"] = st.number_input(
            "Cancel_rate", 0.0, 100.0, default_value(churn_dataset, "Cancel_rate", 0.0)
        )
        values["avg_order_value"] = st.number_input(
            "avg_order_value",
            0.0,
            10000.0,
            default_value(churn_dataset, "avg_order_value", 150.0),
        )
    with col2:
        values["pct_with_color"] = st.number_input(
            "pct_with_color",
            0.0,
            100.0,
            default_value(churn_dataset, "pct_with_color", 25.0),
        )
        values["color_diversity"] = st.number_input(
            "color_diversity",
            0.0,
            10.0,
            default_value(churn_dataset, "color_diversity", 1.5),
        )
        values["is_color_specialist"] = float(st.checkbox("is_color_specialist", value=False))
        values["pct_with_material"] = st.number_input(
            "pct_with_material",
            0.0,
            100.0,
            default_value(churn_dataset, "pct_with_material", 10.0),
        )
        values["pct_purchases_sets"] = st.number_input(
            "pct_purchases_sets",
            0.0,
            100.0,
            default_value(churn_dataset, "pct_purchases_sets", 0.0),
        )
    with col3:
        values["avg_quantity_in_set"] = st.number_input("avg_quantity_in_set", 0.0, 100.0, 0.0)
        values["avg_days_between_purchases"] = st.number_input(
            "avg_days_between_purchases",
            0.0,
            365.0,
            default_value(churn_dataset, "avg_days_between_purchases", 30.0),
        )
        values["months_active"] = st.number_input(
            "months_active",
            1.0,
            12.0,
            default_value(churn_dataset, "months_active", 2.0),
        )
        values["n_products_unique"] = st.number_input(
            "n_products_unique",
            0.0,
            1000.0,
            default_value(churn_dataset, "n_products_unique", 20.0),
        )
    return values
...
def render_simulator(churn_dataset: pd.DataFrame, segmented: pd.DataFrame) -> None:
    ...
    values = collect_simulation_inputs(churn_dataset)
